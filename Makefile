...
upload-snapserver:
	rsync -ahvP --recursive --files-from=snapserver-build-files.txt ./ jonathan@snapserver.local:mopidy-build/

upload:
	rsync -ahvP --recursive --files-from=snapclient-build-files.txt ./ jonathan@snapclient-$(client).local:snapclient-build/
